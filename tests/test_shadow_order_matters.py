...
@pytest.mark.parametrize("reason, kind", [
    ...
    # The real log's wording: the group had 8,017 left, the ticker's own 12%
    # cap 11,994 -- the group bound it, so the order it came in mattered.
...
def test_a_day_says_what_was_skipped_and_whether_it_outranked_a_purchase():
    ...
    events = [
        ...
        om.Event(d2, "TIP", 0.70, "REJECTED", "no room under the 'Duration' group limit"),   # itself: never
    ...
    assert [s.outranks for s in days[1].skipped] == [(), ("XOM",)]      # TIP 0.70 over XOM 0.68, not itself
    out = om.summary(days, cycles)
    assert (out["cycle_days"], out["days"], out["skipped"], out["outranked_days"]) == (3, 2, 3, 2)
    assert out["by_kind"] == {om.GROUP: 1, om.GROSS: 1, om.SLEEVE: 1}
