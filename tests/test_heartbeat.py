...
def _capture_prompt(monkeypatch) -> dict:
    ...
    monkeypatch.setattr(hb, "post_signal", lambda s, client=None: httpx.Response(200, text="{}"))
    return seen
...
def test_successful_cycle_is_journalled(monkeypatch, _journal_to_tmp):
    ...
    monkeypatch.setattr(
        hb, "post_signal",
        lambda s, client=None: httpx.Response(200, json={"status": "ACCEPTED", "reason": "ok", "quantity": 50}),
    )
    ...
    assert entry["outcome"] == {"http_status": 200, "status": "ACCEPTED", "reason": "ok", "quantity": 50, "order_id": None}
...
def test_outcome_survives_a_non_json_error_body(monkeypatch, _journal_to_tmp):
    monkeypatch.setattr(hb, "fetch_news", lambda t: ["news"])
    monkeypatch.setattr(hb, "call_llm", lambda s, u, j: json.dumps({"ticker": "AAPL", "bias": "BULLISH", "conviction": 0.9, "rationale": "r"}))
    monkeypatch.setattr(hb, "post_signal", lambda s, client=None: httpx.Response(502, text="bad gateway"))

    hb.process_ticker("AAPL")
    entry = json.loads(_journal_to_tmp.read_text().splitlines()[0])
    assert entry["outcome"] == {"http_status": 502, "body": "bad gateway"}
