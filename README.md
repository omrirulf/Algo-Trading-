...
| Max 25% per exposure group (Duration 30%) | `risk_engine.exposure_group_headroom()` — what stops a diversified watchlist producing a one-bet book. Spans every sleeve, so XOM plus `XLE` plus two energy funds is one energy bet made four times. Duration's own ceiling is in `config.settings.EXPOSURE_GROUP_CAP_OVERRIDES`, set by what that bucket can lose rather than by the general number |
| Max 60% net stock-market risk, by beta | `risk_engine.equity_risk_headroom()` — what the equity groups share and none of them bounds. Each stock or stock fund counts at market value × beta (`instruments.EQUITY_RISK_BETAS`); longs add and shorts subtract, inside this bucket only |
