...
8. Size = floor((cap × equity − existing exposure) / price), bounded by the tightest of the group, sleeve and gross limits; 0 shares → `REJECTED`. The cap is 5% for a single name, 12% for a broad fund, 8% for a one-sector or one-country fund and 4% for a single-commodity fund, resolved from the ticker.
